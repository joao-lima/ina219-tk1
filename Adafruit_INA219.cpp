...
/**************************************************************************/
void
 Adafruit_INA219::wireWriteRegister(uint8_t reg, uint16_t value)
{
#if 0
...
#endif
	//i2c_smbus_write_word_data(device, reg, value);
	uint8_t data[2];
	data[0] = (value >> 8);
	data[1] = (value & 0xFF);
	i2c_smbus_write_i2c_block_data(device, reg, 2, data);
}
...
#endif
#if 0
	uint8_t hi = i2c_smbus_read_byte_data(device, reg);
	uint8_t lo = i2c_smbus_read_byte_data(device, reg+1);
	*value  = ((hi << 8) | lo);
#endif
	uint8_t data[2];
	i2c_smbus_read_i2c_block_data(device, reg, 2, data);
